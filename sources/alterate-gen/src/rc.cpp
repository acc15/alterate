...
void print_close_braces(ostream& stream, size_t count) {
    while (count > 0) {
        --count;
        stream << "}" << endl;
    }
...
size_t print_namespace_decl(ostream& stream, const char* ns_ptr) {
    if (ns_ptr == nullptr) {
        return 0;
    }
    size_t ns_level = 0;
...
bool is_uptodate(const char* path, const rc_args& args) {
    ...
    while (getline(input_stream, line)) {
        bool has_mark = boost::starts_with(line, "// ###");
        if (!mark_found) {
            mark_found = has_mark;
            continue;
        }
...
void print_header(ostream& out, const vector<const char*>& args) {
    out << "// " << endl;
    ...
    for (vector<const char*>::const_iterator iter = args.begin(); iter != args.end(); iter++) {
        out << "// " << (*iter) << endl;
    }
    out << "// ###" << endl;
    out << "// " << endl << endl;
...
generate_status generate_hpp(const rc_args& args) {
    ...
    print_header(header_stream, args.inputs);
    header_stream << "#pragma once" << endl << endl << "#include <alterate/resource.h>" << endl << endl;

    size_t ns_level = print_namespace_decl(header_stream, args.res_ns);
    header_stream << endl;
    for (rc_args::input_vector::const_iterator iter = args.inputs.begin(); iter != args.inputs.end(); iter++) {
        string input_string(*iter);
        transform(input_string.begin(), input_string.end(), input_string.begin(), &replace_path_char_to_underscore);
        transform(input_string.begin(), input_string.end(), input_string.begin(), &to_upper);
        header_stream << "extern const alterate::resource::locator " << input_string << ";" << endl;
    }
    header_stream << endl;
...
generate_status generate_cpp(const rc_args& args) {
    ...
    print_header(cpp_stream, args.inputs);
    cpp_stream << "#include \"" << args.hpp_path << "\"" << endl << endl << "using namespace alterate;" << endl << endl;

    for (rc_args::input_vector::const_iterator iter = args.inputs.begin(); iter != args.inputs.end(); iter++) {
        string input_string(*iter);
        transform(input_string.begin(), input_string.end(), input_string.begin(), &replace_path_char_to_underscore);
        cpp_stream << "extern byte_t _binary_" << input_string << "_start;" << endl;
        cpp_stream << "extern byte_t _binary_" << input_string << "_end;" << endl;
        cpp_stream << "extern size_t  _binary_" << input_string << "_size;" << endl;
        cpp_stream << endl;
    }

    size_t ns_level = print_namespace_decl(cpp_stream, args.res_ns);
    cpp_stream << endl;
    for (rc_args::input_vector::const_iterator iter = args.inputs.begin(); iter != args.inputs.end(); iter++) {
        string input_string(*iter);
        transform(input_string.begin(), input_string.end(), input_string.begin(), &replace_path_char_to_underscore);
        ...
        cpp_stream << "const alterate::resource::locator " << variable_string << "(" << endl <<
...
bool generate_file(const char* what, const char* path, const rc_args& args, generate_status(*func)(const rc_args&)) {
